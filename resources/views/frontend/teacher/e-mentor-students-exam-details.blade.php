                ...
                <div class="col-12">
                    <div class="d-md-flex align-items-center">
                        ...
                        <div class="position-relative mt-n4 ">
                            <img src="{{ $stData && isset($stData['photo']) && Storage::exists($stData['photo']) ? Storage::url($stData['photo']) : Storage::url('studentDocs/student-profile-photo.png') }}" alt="logo" class="rounded-3 border student-profile-e-mentor">
                        </div>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <div>
                                    ...
                                    @if(Auth()->user()->role == "admin" || Auth()->user()->role == 'superadmin')
                                        <a href="{{route("e-mentors-edit", base64_encode($ementorStudentData['ementorId']))}}" class="btn btn-outline-primary float-end">Back</a>
                                    @else
                                        <a href="{{route('e-mentor-students')}}" class="btn btn-outline-primary">Back</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                                    ...
                                    @if ($courseData != null)
                                        @php
                                            $studentCourseMaster = DB::table('student_course_master')
                                                ->where([
                                                    'id' => $ementorStudentData['studentData'][0]['student_course_master_id'],
                                                ...
                                                <div class="card-body p-4">
                                                    ...
                                                    @php
                                                        $examData = [
                                                            ...
                                                            'courseRelatedData' => $ementorStudentData['courseData'],
                                                            'courseTitle' => $ementorStudentData['courseTitle'],
                                                            'courseId' => $ementorStudentData['courseId'],
                                                            'courseCategory' => $ementorStudentData['courseCategory'],
                                                ...
                                                @php
                                                    ...
                                                    if ($course && $course->category_id != 1) {
                                                        $awardCourseIds = DB::table('master_course_management')
                                                            ...
                                                            ->where(['optional_course_id' => null, 'is_deleted' => 'No'])
                                                            ->pluck('course_id');

                                                        $studentCourseMaster = getData('student_course_master', ['preference_id', 'preference_status'], ['id' => $ementorStudentData['studentData'][0]['student_course_master_id']]);
                                                        ...
                                                        $eportfolioCount = DB::table('exam_eportfolio')
                                                            ...
                                                            ->whereIn('exam_eportfolio.course_id', $awardCourseIds) // Check for multiple course IDs
                                                            ->where('remark', '!=', null) // Ensure remark is not null (passed condition)
                                                            ->select('exam_eportfolio.course_id') // Get the distinct course IDs
                                                            ->distinct()
                                                            ->pluck('course_id')
                                                            ->toArray();

                                                        $hasPassedAll = count($eportfolioCount) === count($awardCourseIds);
                                                        if ($hasPassedAll) {
                                                            $eportfolioCount = 1;
                                                        }else{
