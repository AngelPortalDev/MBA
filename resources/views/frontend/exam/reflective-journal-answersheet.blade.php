...
<main>
    <section class="p-lg-5 py-7">
        <div class="container">
            @php
                $exam = $examData[0];
                $reflectiveJournalExam = $exam['reflective_journal_exam'][0];
                $reflectiveJournalQuestion = $reflectiveJournalExam['reflective_journal_question'];
                $studentData = $exam['exam_student'][0];
                $courseData = $exam['exam_course'][0];
                $totalMarks = 0;
                $totalObtain = 0;
                foreach ($reflectiveJournalQuestion as $value) {
                    $totalMarks += isset($value['marks']) && !empty($value['marks']) ?  $value['marks'] : 0;
                    $totalObtain += isset($value['reflective_journal_answers'][0]['marks_given']) && !empty($value['reflective_journal_answers'][0]['marks_given']) ?  $value['reflective_journal_answers'][0]['marks_given'] : 0;
                }
            @endphp
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body py-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <h3 class="fw-bold mb-0 color-blue ">{{isset($reflectiveJournalExam['title'])  ? html_entity_decode($reflectiveJournalExam['title']) : ''}} ({{isset($reflectiveJournalExam['percentage'])  ? $reflectiveJournalExam['percentage'] : 0}}%) <span
                    ...
                    <div class="card rounded-3">
                        ...
                        <section class="container px-4 pt-4">
                            ...
                            <div class="row justify-content-center">
                                <div class="col-md-12">
                                    @php
                                        echo isset($reflectiveJournalExam['instructions']) ? html_entity_decode($reflectiveJournalExam['instructions']) : '';
                                    @endphp
                                </div>
                                <hr>
                                ...
                                <form id="examMarksForm">
                                    ...
                                    <input type="hidden" name="exam_type"  value="{{ base64_encode(6)}}">
                                    <input type="hidden" name="student_course_master_id"  value="{{ base64_encode($exam['student_course_master_id'])}}">
                                    <input type="hidden" name="type"  value="Reflective Journal">

                                    @foreach($reflectiveJournalQuestion as $key => $reflectiveJournal)
                                        <div class="col-md-12 mb-5">
                                            <label for="textarea-input" class="form-label"></label>

                                            <input type="hidden" name="question_id[]" id="question_id" value="{{ base64_encode($reflectiveJournal['id']) }}">

                                            <h5 class="text-inherit mb-1 reflectivejournalementorquestions">
                                                <span>Question {{$key + 1}}.</span>
                                                <span class="d-inline mb-0" style="float: inline-end">[{{ isset($reflectiveJournal['marks']) ? $reflectiveJournal['marks'] : 0 }} Marks]</span>
                                              
                                               <span class="mb-0"> {!! html_entity_decode($reflectiveJournal['question']) !!}</span>
                                            </h5>

                                            <h5 class="mb-0 color-blue reflectivejournalementoranswer">Answer:</h5>
                                            <p style="white-space: pre-line">{!! isset($reflectiveJournal['reflective_journal_answers'][0]['answer']) ? html_entity_decode($reflectiveJournal['reflective_journal_answers'][0]['answer']) : '--Not Attempted--' !!}</p>

                                            <div class="color-blue d-flex align-items-end mt-3">
                                                <div class="col-md-4 col-lg-2 col-sm-12">
                                                    <h5 class="me-3">Marks: <span>(Out of {{ isset($reflectiveJournal['marks']) ? $reflectiveJournal['marks'] : 0 }})</span></h5>
                                                    <input type="number" name="marks[]" class="form-control marks-input" placeholder="Give Marks" 
                                                        max="{{ isset($reflectiveJournal['marks']) ? $reflectiveJournal['marks'] : 0 }}" 
                                                        value="{{ isset($reflectiveJournal['reflective_journal_answers'][0]['marks_given']) ? $reflectiveJournal['reflective_journal_answers'][0]['marks_given'] : '0' }}" 
                                                        required {{ Auth::user()->role == "instructor" || Auth::user()->role === 'sub-instructor' ? '' : 'disabled' }}>
                                                </div>
                                            </div>
                                        </div>
