...
<main>
    <section class="p-lg-5 py-7">
        <div class="container">
            ...
            @php
                    $exam = $examData[0];
                    $mockExam = $exam['mock_exam'][0];
                    $questions =$mockExam['mock_question'];
                    ...
                    foreach ($questions as $value) {
                        ...
                        $totalObtain += isset($value['mock_anwers'][0]['marks_given']) && !empty($value['mock_anwers'][0]['marks_given']) ?  $value['mock_anwers'][0]['marks_given'] : 0;
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body py-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <h3 class="fw-bold mb-0 color-blue mentor-answersheet-header">{{isset($mockExam['title'])  ? html_entity_decode($mockExam['title']) : ''}} ({{isset($mockExam['percentage'])  ? $mockExam['percentage'] : 0}}%) <span
                    ...
                    <div class="card rounded-3">
                        ...
                        <section class="container px-4 pt-4">
                            ...
                            <form id="examMarksForm">
                                ...
                                <input type="hidden" name="exam_id"  value="{{ base64_encode($mockExam['id'])}}">
                            ...
                            <div class="row justify-content-center">
                                <div class="col-md-12 mb-3">
                                    @php
                                     echo isset($mockExam['instructions']) ? html_entity_decode($mockExam['instructions']) : ""; 
                                     $num = 1;
                                    @endphp
                                    @if (isset($mockExam['instrcution_file_url']) && Storage::exists($mockExam['instrcution_file_url']))
                                    <div class="mockjobdesc">
                                        <a href="{{ Storage::url($mockExam['instrcution_file_url']) }}" target="_blank"> {{isset($mockExam['instrcution_file_name']) ? $mockExam['instrcution_file_name'] : "Instruction File"}} <i class="fe fe-download fs-5"></i></a>
                                    </div>
                                    @endif
                                </div>
                                <hr>
                                ...
                                <div class="col-md-12 mb-5">
                                    ...
                                    <h5 class="mock_interview_ementor_answer">Answer:</h5>
                                    <div class="row mt-2">
                                        <div class="col-md-3 col-12">
                                            @if (isset($item['file_type']) && !empty($item['file_type']) && $item['file_type'] === "1")
                                             @if (isset($item['mock_anwers'][0]['answer_file_url']) && !empty($item['mock_anwers'][0]['answer_file_url']) && Storage::exists($item['mock_anwers'][0]['answer_file_url']))
                                            <div class="d-flex justify-content-center align-items-center rounded border-white border rounded-3 bg-cover" style="background-image:url({{asset('frontend/images/document-thumbnail-01.png') }});height:155px;">
                                                <a class="icon-shape rounded-circle btn-play icon-xl bg-blue" href="javascript:void(0)" onclick="openPdf('{{$item['mock_anwers'][0]['answer_file_url']}}')">
                                             ...
                                             @else
                                                ...
                                                @endif
                                            @elseif(isset($item['file_type']) && !empty($item['file_type']) && $item['file_type'] === "2")
                                                @if (isset($item['mock_anwers'][0]['answer_file_url']) && !empty($item['mock_anwers'][0]['answer_file_url']))
                                                    <div class="d-flex justify-content-center align-items-center rounded border-white border rounded-3 bg-cover" style="background-image:url({{asset('frontend/images/course/mock-interview-thumbnail-01.png') }});height:155px;">
                                                        <a class="glightbox icon-shape rounded-circle btn-play icon-xl" href="https://iframe.mediadelivery.net/embed/{{env('Student_LIBRARY_ID')}}/{{  $item['mock_anwers'][0]['answer_file_url'] }}">
                                        ...
                                        <div class="col-md-12 col-12 mt-3">
                                            <div class="color-blue d-flex align-items-end">
                                                <div class="col-md-4 col-lg-2 col-sm-12">
                                                    <h5 class="me-3 ">Marks: <span>(Out of {{ isset($item['marks']) ? $item['marks'] : ''}})</span></h5>
                                                <input type="number" name="marks[]" class="form-control marks-input" placeholder="Give Marks" max="{{ isset($item['marks']) ? $item['marks'] : 0}}" value="{{ isset($item['mock_anwers'][0]['marks_given']) ? $item['mock_anwers'][0]['marks_given'] : ''}}"  required {{Auth::user()->role == "instructor" || Auth::user()->role === 'sub-instructor' ? '' : 'disabled' }}>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
